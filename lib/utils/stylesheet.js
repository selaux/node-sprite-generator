...
module.exports = {
    ...
    getRelativeSpriteDir: function (options) {
        if (!options.stylesheetPath || !options.spritePath) {
            return options;
        }

        var stylesheetDir = path.dirname(path.resolve(options.stylesheetPath)),
            spriteDir = path.resolve(options.spritePath),
        ...
        return R.evolve({
            stylesheetOptions: R.merge({ spritePath: relativePath }, R.__)
        }, options);
    }
};
